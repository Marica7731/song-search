...
| 页面 | 筛选条件 | 表格/结果 | 复制 |
|---|---|---|---|
| 首页 | 记住来源、排序、页大小和搜索字段，URL 参数优先 | 行级复制改用稳定 `rowId` | 复制字段、格式、分隔符和自定义面板展开状态可记忆 |
| 数据 | 记住 tab、来源、关键词和摘要链接数 | 刷新后回到同一统计视图 | 原有摘要/列表复制继续使用当前视图设置 |
| BV 查重 | 保留全部/非重复筛选 | 高级复制字段折叠，减少主流程干扰 | 复制字段、格式、重复链接位次按 BV 页面保存 |
| 歌名歌手查重 | 保留全部/非重复筛选 | 高级复制字段折叠，歌手不一致状态继续展示 | 复制字段和格式按歌名歌手页面保存 |
| 命名工具 | 新增全部/需要确认/未找到筛选 | 结果区与当前筛选同步 | 支持复制全部、复制当前筛选、复制当前筛选 TSV |
| 日报 | 复制摘要跟随当前区间和指标视图 | 表格复制当前分页 | 新增区间摘要和当前表格页 TSV 复制 |
...
| `index.html` | 首页正式页面 | 记住筛选条件与自定义复制设置，继续使用 `rowId` 做行级复制 | 读取 `/api/search` 和 `/api/search/export` |
