...
- 已采用显式 `canonical + aliases[]` 白名单，不使用全局忽略标点的归一化。
- 已纳入：上方高置信表中的纯符号、空格、大小写、全半角标点、缺闭合符号差异。
- 暂未纳入：`RE: I AM`、`闇のBAROQUE -バロック-`、`灼熱にて純情 (wii-wii-woo)`、`Re:Re:`。这些标题包含短英文冒号、括号副标题或歌手字段差异，需要更精确的歌手+标题联合白名单后再处理。
- 线上 31,320 条数据回放结果：现行去重 6,114，实施后预期去重 6,077；`フクロウ` 保持 47 条 / 去重 1。

## 2026-06-25 身份特化补充

- 新增 `normalizeSongIdentityKey(song)`，只用于唯一曲目统计、搜索去重计数和统计聚合；标题查找索引仍使用 `normalizeSongTitleKey(title)`，避免影响输入查重和标题定位。
- `ワールドイズマイン` 特化为 2 组：
  - `ワールドイズマインCPK! Remix - かぐや(cv.夏吉ゆうこ), 月見ヤチヨ(cv.早見沙織)`：标题含 `CPK! Remix`，或标题/歌手含 `超かぐや姫`、`かぐや`、`月見ヤチヨ`、`夏吉ゆうこ`、`早見沙織`。
  - `ワールドイズマイン - ryo(supercell) feat.初音ミク`：其余普通原曲系 `ryo`、`supercell`、`初音ミク` 变体。
- `ray` 特化为 2 组：
  - `ray（超かぐや姫！Ver.） - かぐや(cv.夏吉ゆうこ), 月見ヤチヨ(cv.早見沙織)`：标题含 `超かぐや姫`，或标题/歌手含 `かぐや`、`月見ヤチヨ`、`夏吉ゆうこ`、`早見沙織`。
  - `ray - BUMP OF CHICKEN`：标题精确为 `ray` 且不含超かぐや姫信号的普通原曲系。
- 不把 `CPK`、`超かぐや姫`、`CV` 或 `声優` 规则扩展到其它歌曲；`メルト CPK! Remix`、`星降る海` 等相似候选需要单独人工确认。

## 2026-07-21 追加

- 已纳入 `どこまでも~How Far I'll Go~`：覆盖 `どこまでも ~How Far I'll Go~`、`どこまでも ～How Far I'll Go～`、`どこまでも～How FarI'll Go～`、`How Far I'll Go` 等高置信表记。
- 同步修复搜索拆词里英文词内撇号的处理，并让搜索/统计关键词比较兼容波浪号两侧空格和紧凑英文写法，避免 `How Far I'll Go` 或 `どこまでも~How Far I'll Go` 被拆成错误查询条件。

## 2026-07-22 追加

- `炉心融解` 作为标题级同曲处理，不再因 `iroha`、`iroha(sasaki)`、`iraha(sasaki) feat. 鏡音リン` 的原唱字段差异导致全库预计算的“独特曲目”误判。

## 2026-06-25 月陽与同类符号补充

- `月陽`：`月陽-ツキアカリ-`、`月陽 -ツキアカリ-` 归为同一标题 key。
- 红叶丸已封存，`BV1JBoXB4Enu` 无法重新解析；按人工确认的真实分 P 修正封存数据，其中 P1 从 `月陽 -ツキアカリ- / りぶ` 改为 `月陽-ツキアカリ- / みきとP feat.GUMI`。
- 同批只补高置信同版本符号差异白名单：`Butter-Fly ～tri.Version～`、`さよならの夏～コクリコ坂から～`、`貴方解剖純愛歌~死ね~`、`Swallowtail Butterfly 〜あいのうた〜`、`ETERNAL WIND ～ほほえみは光る風の中～`、`Let It Go ～ありのままで～`、`恋におちて-Fall in Love-`、`Time after time 〜花舞う街で〜`、`Falco-ファルコ-`。
- 未纳入 `ベリーメロン`，因为线上歌手表记不稳定，需要单独人工确认。
