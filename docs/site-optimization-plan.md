...
- 2026-05-18 继续调整结果卡层级：来源改为淡化 meta，歌手使用次级强调色；移动端行按钮取消文字，只保留图标和无障碍标签；恢复高频的 B站编辑按钮，并把复制链接放在编辑按钮右侧。
...
| `index.html` | 正式首页 | 使用 `/api/bootstrap`、`/api/search`、`/api/search/export` 展示搜索、来源筛选、桌面复制和 H5 底部弹层；移动端结果卡控制 meta、合集和操作按钮高度，并提供编辑、复制链接、打开B站三个行级动作 | 公网主入口 |
