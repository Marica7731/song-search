...
- `index.html`：优化版歌库首页，支持可搜索来源、服务端分页、搜索范围切换、筛选偏好记忆、复制预设、自定义复制记忆、稳定行级复制和 H5 响应式布局。
...
- 首页：来源、排序、页大小、搜索字段会保存在浏览器本地；自定义复制字段、格式、分隔符和展开状态也会记忆。
...
```text
首页：调整来源、排序、字段或复制设置后，刷新页面会保留设置；URL 参数仍优先生效。
