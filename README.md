...
- `index.html`：优化版歌库首页，支持可搜索来源、服务端分页、搜索范围切换、筛选偏好记忆、紧凑复制工具条、自定义复制记忆、字段点击复制、稳定行级复制和 H5 响应式布局。
...
- `title-artist-check.html`：命名和校验工具，支持未命中项改名重查、搜索辅助、服务端候选摘要、结果筛选和表格复制。
...
- 首页：来源、排序、页大小、搜索字段会保存在浏览器本地；复制区改为“复制类型下拉 + 复制按钮 + 字段设置”，自定义复制字段、格式、分隔符、复制类型和展开状态都会记忆；结果里的歌名、歌手、合集、来源、BV 和日期可直接点击复制。
- 首页来源列表：来源数量来自服务端 `/api/bootstrap` 的 `sourceStats.totalSongs`，并兼容旧的 `count/total` 字段，避免单来源数量显示为 0。
- 数据页：会记住当前 tab、来源筛选、关键词和摘要链接数，刷新后继续停留在同一工作视图。
- 正式页共享资源：`/stats`、`/bv`、`/dup`、`/check`、`/growth` 均通过带版本号的 `site-theme.css`、`site-shell.js`、`last-run-badge.js` 加载统一 UI；页面首屏已经是静态共享外壳，避免先显示旧页面再由 JS 套壳。
- 数据页视觉：已移除旧右侧目录浮层和技术化更新文案，统一为首页同款侧栏、分段 tab、筛选栏、概览卡和列表卡样式。
- BV 查重 / 歌名歌手查重：常用复制预设保留在主操作区，高级字段折叠到“高级复制字段”，并按页面分别保存复制字段、格式和重复链接位次。
- 命名工具：候选结果支持“全部 / 需要确认 / 未找到”筛选并用颜色区分；“需要确认”覆盖用户歌手与库中候选不一致、或用户未提供歌手；“未找到”提示用户自行找歌手；支持逐个打开当前筛选的网易云搜索，并可在有用户输入歌手时勾选“不带歌手”。
- 日报：新增“复制摘要”和“复制当前表格”，摘要按当前区间和当前指标视图生成，表格复制当前分页 TSV。
- 数据页 / BV 查重 / 歌名歌手查重 / 命名工具 / 日报：正式入口已接入共享页面外壳，点击首页左侧导航不再进入旧的文字导航页面；桌面端使用左侧主导航，H5 使用顶部横向导航。

使用方法：

```text
首页：在复制下拉里选择“本页：歌名 - 歌手”“本页：歌名 - 歌手 + 链接”“本页 TSV”“全部结果”或“自定义字段”，再点“复制”；点“字段”展开自定义字段设置；点结果里的歌名、歌手、合集、来源、BV 或日期可单独复制。
数据页：切换 tab、来源或搜索后，刷新保留当前视图。
查重页：直接点复制预设；需要改字段时展开“高级复制字段”。
命名工具：校验后用筛选按钮收敛结果，再按需要复制全部、当前筛选或表格。
日报：切换区间或指标后复制摘要；翻页后复制当前表格页。
全站导航：从首页点击“统计 / BV 查重 / 歌名查重 / 命名校验 / 增长日报”会进入统一外壳页面，当前 tab 会高亮。
```
