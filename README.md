...
- `bv-dup-check.html`：BV 批量查重，支持服务端 live fallback 上限、分组摘要、复制预设、高级复制字段折叠、复制设置记忆和自适应多列结果卡。
...
- BV 查重 / 歌名歌手查重：常用复制预设保留在主操作区，高级字段折叠到“高级复制字段”，并按页面分别保存复制字段、格式和重复链接位次；来源选择移入左侧栏并显示来源头像、投稿数和去重数；输入框使用主内容宽度，右侧目录进入布局列而不是覆盖主内容；结果区按可用宽度自适应多列卡片，单卡压缩封面、标题、歌手、来源和重复链接预览，首次 / 重复 / 未找到 / 歌手疑似不一致使用不同颜色标记；歌名歌手查重页的输入区改为短 placeholder 和独立格式提示，避免空页面显示粗重滚动条。
...
查重页：左侧栏切换来源，主内容区批量输入 BV 或“歌名 - 歌手”后分析；结果会按宽度排成多列卡片，卡里的标题、歌手和来源仍可点击复制，封面和链接可打开 B 站；直接点复制预设，需要改字段时展开“高级复制字段”。
