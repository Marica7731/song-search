...
- 命名工具：候选结果支持“全部 / 需要确认 / 未找到”筛选并用颜色区分；“需要确认”覆盖用户歌手与库中候选不一致、或用户未提供歌手；“未找到”提示用户自行找歌手；选择候选歌手会同步到修正输入框和网易云链接；复制按钮只复制当前可见结果；批量网易云只打开当前可见的待处理项，并可在有用户输入歌手时勾选“不带歌手”。
...
命名工具：校验后用筛选按钮收敛结果，点“复制”复制当前可见结果；需要外部查找时只对待处理项逐个打开网易云搜索。
