...
- `index.html`：优化版歌库首页，支持可搜索来源、来源头像、BV 封面缩略图、服务端分页、搜索范围切换、最近更新时间展示、独特歌曲星标、刷新默认筛选、桌面紧凑复制工具条、桌面自适应多列结果卡、H5 筛选/复制底部弹层、字段点击复制、稳定行级复制和移动端紧凑结果卡。
- `index-optimized.html`：优化首页的对照文件，与正式首页保持同源，便于后续继续调样式或回看方案。
- `stats.html`：数据统计页，展示来源、歌手、曲目、投稿时间等统计视图，来源分组补充场次、曲数、去重、场均和独特歌曲指标；其中“场次”指去重 BV 号数量，“曲数”指歌曲条目数，“场均”是曲数 / BV 场次。桌面端使用左侧导航、主内容、右侧 sticky 目录三栏布局，来源分组、歌曲排行和歌手聚合使用自适应多列卡片，窄屏目录折叠为可关闭抽屉。
...
- `song-growth.html`：歌曲总量日报和增长趋势，读取服务端缓存后的增长数据，支持播放量、曲目、去重歌曲三维度分析；页面内有独立“去重歌曲曲线”面板，按累计去重歌曲和当日去重增量展示趋势，也可通过带头像的来源选择器查看单个来源累计去重歌曲曲线，并支持复制区间摘要和当前表格页。
...
- 数据页：左侧新增“数据导航”用于切换来源统计、歌曲排行和歌手聚合；来源筛选保留在内容区作为筛选条件；tab、来源、关键词和摘要链接数只在当前页面会话内生效，刷新后回到默认来源统计视图。
...
- 日报：新增独立“去重歌曲曲线”面板，绿色折线显示累计去重歌曲，蓝色柱形显示当日去重增量；面板上方可以选择全部来源或单个来源，选择器和选项列表都会显示来源头像、累计去重曲目和当前区间增量；日报页不再挂载全站导航和页面目录，减少分析页干扰。
- 数据页 / BV 查重 / 歌名歌手查重 / 命名工具：正式入口已接入共享页面外壳，左上角统一为纯文字 `culua.com`；桌面端使用左侧主导航，H5 使用紧凑顶部横向导航，并隐藏重复说明标签和数据页内部重复 tab，减少首屏占用。
...
日报：切换区间或指标后查看播放量、曲目、去重歌曲或商值分析；“去重歌曲曲线”始终单独显示累计去重和日新增去重，点带头像的来源选择器可切换单来源曲线，翻页后可复制当前表格页。
