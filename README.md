...
- `index.html`：优化版歌库首页，支持可搜索来源、来源头像、BV 封面缩略图、服务端分页、搜索范围切换、最近更新时间展示、筛选偏好记忆、桌面紧凑复制工具条、桌面自适应多列结果卡、H5 筛选/复制底部弹层、字段点击复制、稳定行级复制和移动端紧凑结果卡。
- `index-optimized.html`：优化首页的对照文件，与正式首页保持同源，便于后续继续调样式或回看方案。
- `stats.html`：数据统计页，展示来源、歌手、曲目、投稿时间等统计视图，并记住 tab、来源、搜索词和摘要链接数；桌面端使用左侧导航、主内容、右侧 sticky 目录三栏布局，来源分组、歌曲排行和歌手聚合使用自适应多列卡片，窄屏目录折叠为可关闭抽屉。
...
- `song-growth.html`：歌曲总量日报和增长趋势，读取服务端缓存后的增长数据，支持播放量、曲目、去重歌曲三维度分析；页面内有独立“去重歌曲曲线”面板，按累计去重歌曲和当日去重增量展示趋势，并支持复制区间摘要和当前表格页。
...
- 首页：排序、页大小、搜索字段会保存在浏览器本地；来源只作用于当前页面会话，刷新或重新访问默认回到全部来源；桌面端复制区为“复制类型下拉 + 复制按钮 + 字段设置”，H5 端改为“复制”底部弹层；自定义复制字段、格式、分隔符、复制类型和展开状态都会记忆；结果里的歌名、歌手、合集、来源、BV 和日期可直接点击复制。
- 首页 H5：首屏只保留搜索框、搜索按钮、筛选 chips、摘要和结果列表；来源、排序、搜索范围、页大小进入“筛选”底部弹层；统计卡和桌面复制面板在移动端隐藏；结果卡把来源淡化为带头像的 meta chips、歌手强化为铺满信息列的次级 chip，缩略图固定在左侧，来源/日期/BV 信息压到封面图下方，减少右侧内容空白并压缩卡片高度。
- 首页桌面：结果列表改为自适应多列卡片，复用 H5 的封面、meta chips、歌手 chip 和紧凑图标操作；宽屏会按 1/2、3/4 的自然顺序排卡，保留所有字段点击复制、编辑稿件、复制链接和打开 B 站入口。
...
- BV 查重 / 歌名歌手查重：常用复制预设保留在主操作区，高级字段折叠到“高级复制字段”，并按页面分别保存复制字段、格式和重复链接位次；来源选择移入左侧栏并显示来源头像、投稿数和去重数；输入框使用主内容宽度，右侧目录进入布局列而不是覆盖主内容；结果卡展示 B 站封面、来源头像和链接，首次 / 重复 / 未找到 / 歌手疑似不一致使用不同颜色标记；歌名歌手查重页的输入区改为短 placeholder 和独立格式提示，避免空页面显示粗重滚动条。
...
- 日报：新增独立“去重歌曲曲线”面板，绿色折线显示累计去重歌曲，蓝色柱形显示当日去重增量，并保留去重 KPI 和趋势分析；“复制摘要”和“复制当前表格”会按当前区间、当前指标视图和当前分页生成。
...
查重页：左侧栏切换来源，主内容区批量输入 BV 或“歌名 - 歌手”后分析；结果卡里的标题、歌手和来源仍可点击复制，封面和链接可打开 B 站；直接点复制预设，需要改字段时展开“高级复制字段”。
命名工具：校验后用“已确认 / 需要确认 / 缺歌手 / 待入库 / 未找到”筛选按钮收敛结果，点“复制”复制当前可见结果；需要外部查找时只对待处理项逐个打开网易云搜索。
日报：切换区间或指标后查看播放量、曲目、去重歌曲或商值分析；“去重歌曲曲线”始终单独显示累计去重和日新增去重，翻页后可复制当前表格页。
全站导航：从首页点击“统计 / BV 查重 / 歌名查重 / 命名校验 / 增长日报”会进入统一外壳页面，当前 tab 会高亮；H5 顶部导航可横滑，数据页用第二行“来源统计 / 歌曲排行 / 歌手聚合”直接切换视图。
```

注意事项：

- 排序、页大小、搜索字段和复制设置只写入浏览器 `localStorage`，不写入服务器、不影响其他用户；来源不写入本地偏好，刷新默认回到全部来源。
- URL 查询参数可保留搜索词、排序、分页和页大小；来源不写入 URL，避免刷新后继续停留在上次来源。
...
- 来源头像在 `scripts/source-profiles.json` 里补充；没有头像 URL 时自动使用来源名首个有效字符生成单字头像；已有头像时不再叠加首字，全部来源使用 `ALL` 作为兜底头像文本；数据页的页面目录会同步显示来源头像，便于在长列表里快速定位来源。
...
GET  /api/song-growth         返回 combinedRows、publishUniqueRows、anomalies、cache；combinedRows 包含 uniqueSongTotal/uniqueSongDelta，并按 SONG_GROWTH_CACHE_TTL_MS 缓存
...
390px H5：右侧目录应隐藏为“目录”按钮，抽屉可打开/关闭；分组头部、歌曲行和展开投稿链接不应横向溢出。
```
