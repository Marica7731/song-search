...
- `stats.html`：数据统计页，展示来源、歌手、曲目、投稿时间等统计视图，来源分组补充场次、去重、场均和独特歌曲指标；桌面端使用左侧导航、主内容、右侧 sticky 目录三栏布局，来源分组、歌曲排行和歌手聚合使用自适应多列卡片，窄屏目录折叠为可关闭抽屉。
...
- `song-growth.html`：歌曲总量日报和增长趋势，读取服务端缓存后的增长数据，支持播放量、曲目、去重歌曲三维度分析；页面内有独立“去重歌曲曲线”面板，按累计去重歌曲和当日去重增量展示趋势，也可选择单个来源查看该来源累计去重歌曲曲线，并支持复制区间摘要和当前表格页。
...
- 日报：新增独立“去重歌曲曲线”面板，绿色折线显示累计去重歌曲，蓝色柱形显示当日去重增量；面板上方可以选择全部来源或单个来源，单来源会显示来源头像、累计去重曲目和当前区间增量；“复制摘要”和“复制当前表格”会按当前区间、当前指标视图和当前分页生成。
