...
- `site-shell.js`：正式工具页共享外壳，把数据、查重、命名和日报页面接入与首页一致的侧边导航和 H5 顶部导航。
...
- 正式页共享资源：`/stats`、`/bv`、`/dup`、`/check`、`/growth` 均通过带版本号的 `site-theme.css`、`site-shell.js`、`last-run-badge.js` 加载统一 UI，避免公网缓存继续显示旧壳层或旧文案。
- 数据页视觉：已移除旧右侧目录浮层和技术化更新文案，统一为首页同款侧栏、分段 tab、筛选栏、概览卡和列表卡样式。
- BV 查重 / 歌名歌手查重：常用复制预设保留在主操作区，高级字段折叠到“高级复制字段”，并按页面分别保存复制字段、格式和重复链接位次。
- 命名工具：候选结果支持“全部 / 需要确认 / 未找到”筛选；复制支持全部结果、当前筛选结果和当前筛选表格。
