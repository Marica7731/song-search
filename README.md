...
- `bv-dup-check.html`：BV 批量查重，支持服务端 live fallback 上限、短输入工作区、当前库/结果概览、分组摘要、复制预设、高级复制字段折叠、重复链接位次选择、复制设置记忆和自适应多列结果卡。
- `title-artist-dup-check.html`：按“歌名 - 歌手”批量查重，支持歌手疑似不一致分组、复制预设、高级复制字段折叠、重复链接位次选择、复制设置记忆和自适应多列结果卡。
...
- BV 查重 / 歌名歌手查重：常用复制预设保留在主操作区，高级字段折叠到“高级复制字段”，并按页面分别保存复制字段、格式和重复链接位次；复制链接位次支持正数从前取、负数从后取，例如 `1` 取第一条、`-1` 取最后一条，越界会夹到可用范围；来源选择移入左侧栏并显示来源头像、投稿数和去重数；BV 查重页首屏使用短输入工作区，右侧展示当前库、输入、重复、非重复和未找到概览，目录只保留关键分区；结果区按可用宽度自适应多列卡片，单卡压缩封面、标题、歌手、来源和重复链接预览，BV 查重单卡默认只展示前 2 条重复链接并提示剩余数量，且预览区保留稳定高度，首次 / 重复 / 未找到 / 歌手疑似不一致使用不同颜色标记；歌名歌手查重页的输入区改为短 placeholder 和独立格式提示，避免空页面显示粗重滚动条。
...
查重页：左侧栏切换来源，主内容区批量输入 BV 或“歌名 - 歌手”后分析；BV 查重会在输入区旁显示当前范围和结果概览；结果会按宽度排成多列卡片，卡里的标题、歌手和来源仍可点击复制，封面和链接可打开 B 站；直接点复制预设，需要改字段或链接位次时展开“高级复制字段”，链接位次填 `-1` 可复制最后一条重复链接。
