...
- `index.html`：优化版歌库首页，支持可搜索来源、来源头像、BV 封面缩略图、服务端分页、搜索范围切换、最近更新时间展示、筛选偏好记忆、桌面紧凑复制工具条、H5 筛选/复制底部弹层、字段点击复制、稳定行级复制和移动端紧凑结果卡。
- `index-optimized.html`：优化首页的对照文件，与正式首页保持同源，便于后续继续调样式或回看方案。
- `stats.html`：数据统计页，展示来源、歌手、曲目、投稿时间等统计视图，并记住 tab、来源、搜索词和摘要链接数；桌面端使用左侧导航、主内容、右侧 sticky 目录三栏布局，窄屏目录折叠为可关闭抽屉。
...
- 数据页视觉：统一为首页同款侧栏、分段 tab、筛选栏、概览卡和列表卡样式；桌面端改为左侧导航、主内容、右侧目录三栏，右侧目录挂载进布局流并使用 `position: sticky`，不再覆盖“总投稿 / 去重 / 独特”和复制按钮；来源目录读取来源头像，来源/歌手分组头部使用统计 badge，H5 下统计 chips 固定在分组头部右上角以减少卡片高度，歌曲行使用“文本区 + 操作区”网格，展开投稿链接限制高度并可滚动；H5 和窄屏通过“目录”按钮打开抽屉。
