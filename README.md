...
- BV 查重 / 歌名歌手查重：常用复制预设保留在主操作区，高级字段折叠到“高级复制字段”，并按页面分别保存复制字段、格式和重复链接位次；来源选择移入左侧栏并显示来源头像、投稿数和去重数；BV 查重页首屏使用短输入工作区，右侧展示当前库、输入、重复、非重复和未找到概览，目录只保留关键分区；结果区按可用宽度自适应多列卡片，单卡压缩封面、标题、歌手、来源和重复链接预览，BV 查重单卡默认只展示前 3 条重复链接并提示剩余数量，首次 / 重复 / 未找到 / 歌手疑似不一致使用不同颜色标记；歌名歌手查重页的输入区改为短 placeholder 和独立格式提示，避免空页面显示粗重滚动条。
