...
- 数据页来源统计：VTuber 来源歌曲行可点击展开全部投稿链接，显示去重视频数，并提供“复制链接”快速复制该歌曲的所有投稿；歌曲封面优先使用最新投稿封面。
...
```text
Windows 日语 IME：在 /、/stats、/bv、/dup、/check、/vocaloid、/growth 的搜索框、筛选框或多行输入框里输入 にほんご / konnichiha，按空格应能唤起候选框。
组合输入期间 Enter / Ctrl+Enter 不应触发搜索、跳页或阻止候选确认；组合结束后搜索框和筛选框再执行原有自动搜索或列表刷新。
```
