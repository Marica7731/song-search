...
- `song-growth.html`：歌曲总量日报和增长趋势，读取服务端缓存后的增长数据，支持播放量、曲目、去重歌曲三维度分析，并支持复制区间摘要和当前表格页。
...
- 日报：新增去重歌曲数曲线、去重 KPI 和趋势分析；“复制摘要”和“复制当前表格”会按当前区间、当前指标视图和当前分页生成。
...
日报：切换区间或指标后查看播放量、曲目、去重歌曲或商值分析并复制摘要；翻页后复制当前表格页。
