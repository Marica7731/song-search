...
- WDC 只通过 `deploy/wdc/` 中的 systemd 分发器检查并触发 `update.yml`；不要在 WDC 克隆老站或运行 Puppeteer。
...
- `reports/github-bv-sampling-state.json` 是不提交的抽样状态；不得提交缓存、日志、Cookie、Token、二维码、浏览器配置或其他运行态文件。
...
- 数据更新由 WDC 空闲分发或手动 `workflow_dispatch` 触发；不要另建周期调度。
